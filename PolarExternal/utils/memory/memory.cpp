...
std::uint32_t mem::memory::find_process_id(const std::string& process_name)
{
	...
	if (Process32First(snapshot, &process_entry))
	{
		do
		{
			if (!strcmp(process_name.c_str(), process_entry.szExeFile))
			{
...
std::uint64_t mem::memory::find_module_address(const std::string& module_name)
{
...
bool mem::memory::attach_to_process(const std::string& process_name)
{
...
std::string mem::memory::read_string(std::uint64_t address)
{
	...
	ReadProcessMemory(process_handle, reinterpret_cast<void*>(string_address), buffer.data(), buffer.size(), nullptr);

	return std::string(buffer.data(), string_length);
}

std::uint32_t mem::memory::get_process_id()
{
	return process_id;
}

std::uint64_t mem::memory::get_module_address()
{
	return base_address;
}

HANDLE mem::memory::get_process_handle()
{
